...
public static class DTGUIHelper {
    ...
    public static readonly Color InactiveHeaderColor = new Color(.5f, .5f, .5f);
    public static readonly Color OuterGroupBoxColor = new Color(.7f, 1f, 1f);
    public static readonly Color ActiveHeaderColor = new Color(.3f, .8f, 1f);
    public static readonly Color SecondaryHeaderColor = new Color(.8f, .8f, .8f);
    public static readonly Color GroupBoxColor = new Color(.6f, .6f, .6f);
    public static readonly Color SecondaryGroupBoxColor = new Color(.5f, .8f, 1f);
    public static readonly Color BrightButtonColor = Color.cyan;
    public static readonly Color DragAreaColor = Color.yellow;
    public static readonly Color BrightTextColor = Color.yellow;
    ...
    public static void StartGroupHeader(int level = 0) {
        switch (level) {
            case 0:
    ...
    public static float DisplayVolumeField(float vol, VolumeFieldType fieldType, bool wideMode, float volumeMin = 0f, bool showFieldName = false, string fieldName = "Volume") {
        if (!MasterAudio.UseDbScaleForVolume) {
            switch (fieldType) {
                ...
                case VolumeFieldType.Bus:
                    return GUILayout.HorizontalSlider(vol, 0f, 1f, GUILayout.Width(wideMode ? WideModeWidth : NormalBusWidth));
                case VolumeFieldType.PlaylistController:
                    return GUILayout.HorizontalSlider(vol, 0f, 1f, GUILayout.Width(74));
                case VolumeFieldType.None:
                    if (showFieldName) {
                        return EditorGUILayout.Slider(fieldName, vol, volumeMin, 1f);
                    } else {
                        return EditorGUILayout.Slider(vol, volumeMin, 1f, GUILayout.Width(252));
                    }
            }
        }
        ...
        switch (fieldType) {
            ...
            case VolumeFieldType.Bus:
                ...
                break;
            case VolumeFieldType.PlaylistController:
                newDbLevel = GUILayout.HorizontalSlider(dbLevel, MinDb, MaxDb, GUILayout.Width(74));
                break;
            case VolumeFieldType.None:
                // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                if (showFieldName) {
                    ...
                    newDbLevel = EditorGUILayout.Slider(dbLevel, MinDb, MaxDb, GUILayout.Width(252));
                }
                break;
        }
    ...
    public static DTFunctionButtons AddPlaylistControllerSetupButtons(PlaylistController controller, string itemName, bool jukeboxMode) {
        ...
        if (!jukeboxMode) {
            ...
            var goPressed = GUILayout.Button(new GUIContent(settingsIcon, "Click to edit " + itemName), EditorStyles.toolbarButton);
            var removePressed = false;
