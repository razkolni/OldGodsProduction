...
public class MasterAudioClipManager : EditorWindow {
    ...
    private List<AudioInfo> _filterClips;
    private List<AudioInfo> _filteredOut;
    ...
    void OnGUI() {
        ...
        if (_clipList.AudioInformation.Count == 0 || _clipList.NeedsRefresh) {
        ...
        var totalClips = _clipList.AudioInformation.Count;
    ...
    private bool LoadAndTranslateFile() {
        ...
        if (_clipList.AudioInformation.Count == 0) {
            _clipList.AudioInformation.Clear();
        }
    ...
    private List<AudioInfo> FilteredClips {
        get {
            ...
            _filterClips = new List<AudioInfo>();

            if (!string.IsNullOrEmpty(_clipList.SearchFilter)) {
                if (_filteredOut == null) {
                    _filteredOut = new List<AudioInfo>();
                    _filteredOut.AddRange(_clipList.AudioInformation);
                }

                _filteredOut.RemoveAll(delegate(AudioInfo obj) {
            ...
            var arrayToAddFrom = _clipList.AudioInformation;
            if (_filteredOut != null) {
            ...
            foreach (var aClip in _clipList.AudioInformation) {
    ...
    private void DisplayClips() {
        ...
        if (GUILayout.Button("None", EditorStyles.toolbarButton, GUILayout.Width(36))) {
            foreach (var t in _clipList.AudioInformation) {
                t.IsSelected = false;
            }
        }
    ...
    private static void RevertChanges(AudioInfo info) {
    ...
    private bool TranslateFromXml(XmlDocument xDoc) {
        ...
        try {
            ...
            for (var i = 0; i < files.Count; i++) {
                ...
                var matchingClip = _clipList.AudioInformation.Find(delegate(AudioInfo obj) {
                    return obj.FullPath == path;
                });

                if (matchingClip == null) {
                    var aud = new AudioInfo(path, clipName, is3D, compressionBitrate, forceMono, format, loadType);
                    _clipList.AudioInformation.Add(aud);
                } else {
            ...
            _clipList.AudioInformation.RemoveAll(delegate(AudioInfo obj) {
    ...
    private void BuildCache() {
        ...
        foreach (var aPath in filePaths) {
            ...
            audioInfo.AudioInformation.Add(newClip);
        }

        audioInfo.AudioInformation.RemoveAll(delegate(AudioInfo obj) {
    ...
    private bool WriteFile(AudioInfoData audInfo) {
        ...
        try {
            ...
            foreach (var aud in audInfo.AudioInformation) {
    ...
    private static void SelectClip(AudioInfo info) {
    ...
    public class AudioInfoData {
        public List<AudioInfo> AudioInformation = new List<AudioInfo>();
