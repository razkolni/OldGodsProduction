...
public class MasterAudio : MonoBehaviour {
    ...
    public const string GroupTemplateFolder = MasterAudioFolderPath + "/Sources/Prefabs/GroupTemplates/";
    public const string AudioSourceTemplateFolder = MasterAudioFolderPath + "/Sources/Prefabs/AudioSourceTemplates/";
    ...
    private const string UseDbKey = "~MA_UseDbScaleForVolume~";
    private const string UseCentsPitchKey = "~MA_UseCentsForPitch~";
    private const string StoredLanguageNameKey = "~MA_Language_Key~";
    ...
    void Awake() {
        if (FindObjectsOfType(typeof(MasterAudio)).Length > 1) {
            Destroy(gameObject);
            Debug.Log("More than one Master Audio prefab exists in this Scene. Destroying the newer one called '" + name + "'. You may wish to set up a Bootstrapper Scene so this does not occur.");
            return;
        }
            ...
            for (var i = 0; i < parentGroup.childCount; i++) {
                ...
                for (var j = 0; j < weight; j++) {
                    if (j > 0) {
                        ...
                        if (childVariation.audLocation == AudioLocation.ResourceFile) {
    ...
    IEnumerator CoUpdate() {
        while (true) {
            ...
            if (IgnoreTimeScale) {
                yield return StartCoroutine(CoroutineHelper.WaitForActualSeconds(InnerLoopCheckInterval));
            } else {
    ...
    private static PlaySoundResult PlaySoundAtVolume(string sType,
        ...
        if (group.Group.childGroupMode == MasterAudioGroup.ChildGroupMode.TriggerLinkedGroupsWhenRequested && !triggeredAsChildGroup) {
            // ReSharper disable once ForCanBeConvertedToForeach
        ...
        if (randomSource == null) { // we must get a non-busy random source!
            ...
            if (isNonSpecific) {
                ...
                switch (@group.Group.curVariationSequence) {
        ...
        if (randomSource.Variation.audLocation == AudioLocation.Clip && randomSource.Variation.VarAudio.clip == null) {
            if (loggingEnabledForGrp) {
                LogMessage(string.Format("Child named '{0}' of {1} has no audio assigned to it so nothing will be played.",
                                         randomSource.Variation.name,
                                         sType));
            }

            RemoveClipAndRefillIfEmpty(group, isNonSpecific, randomIndex, choices, sType, pickedChoice, loggingEnabledForGrp);

            return null; // nothing played, it's silent. Don't take up voices because that could silence a Dialog Group that already has a real sound playing.
        }
        ...
        do {
            playedState = PlaySoundIfAvailable(randomSource, sourcePosition, volumePercentage, ref forgetSoundPlayedOrScheduled, pitch, group, sourceTrans, attachToSource, delaySoundTime, useVector3, makePlaySoundResult, isChaining, isSingleSubscribedPlay);
        ...
        if (!soundSuccess) {
            ...
            if (group.Group.childGroupMode == MasterAudioGroup.ChildGroupMode.TriggerLinkedGroupsWhenPlayed && !triggeredAsChildGroup && !IsWarming) {
                // ReSharper disable once ForCanBeConvertedToForeach
                for (var i = 0; i < group.Group.childSoundGroups.Count; i++) {
                    var childGrpName = group.Group.childSoundGroups[i];
                    PlaySoundAtVolume(childGrpName, volumePercentage, sourcePosition, pitch, sourceTrans, null, attachToSource, delaySoundTime, useVector3, false, false, false, true);
    ...
    public static void FadeSoundGroupToVolume(string sType, float newVolume, float fadeTime, Action completionCallback = null) {
        ...
        var volStep = (newVolume - aGroup.groupMasterVolume) / (fadeTime / InnerLoopCheckInterval);
    ...
    public static void RemoveSoundGroup(Transform groupTrans) {
        ...
        for (var i = 0; i < groupTrans.childCount; i++) {
            ...
            if (aVar.audLocation != AudioLocation.ResourceFile) {
                continue;
            }
    ...
    public static Transform CreateNewSoundGroup(DynamicSoundGroup aGroup, string creatorObjectName, bool errorOnExisting = true) {
            ...
            for (var j = 0; j < aVariation.weight; j++) {
                ...
                switch (aVariation.audLocation) {
                    ...
                    case AudioLocation.ResourceFile:
                        ...
                        variation.audLocation = AudioLocation.ResourceFile;
        ...
        if (groupScript.BusForGroup.isMuted) {
    ...
    public static void SetGroupVolume(string sType, float volumeLevel) {
        ...
        for (var i = 0; i < theGroup.Sources.Count; i++) {
            ...
            if (aInfo.Variation.randomVolumeMode == SoundGroupVariation.RandomVolumeMode.AddToClipVolume) {
                newVol = (aInfo.OriginalVolume * aInfo.LastPercentageVolume * aGroup.groupMasterVolume * busVolume * Instance._masterAudioVolume) + aInfo.LastRandomVolume;
            } else { // ignore original volume
                newVol = (aInfo.OriginalVolume * aInfo.LastPercentageVolume * aGroup.groupMasterVolume * busVolume * Instance._masterAudioVolume);
            }
    ...
    public static void FadeBusToVolume(string busName, float newVolume, float fadeTime, Action completionCallback = null) {
        ...
        var volStep = (newVolume - bus.volume) / (fadeTime / InnerLoopCheckInterval);
    ...
    public static void AddCustomEventReceiver(ICustomEventReceiver receiver, Transform receiverTrans) {
        ...
        for (var i = 0; i < Instance.customEvents.Count; i++) {
            ...
            if (!ReceiversByEventName.ContainsKey(anEvent.EventName)) {
                ReceiversByEventName.Add(anEvent.EventName, new Dictionary<ICustomEventReceiver, Transform> {
    ...
    public static float MasterVolumeLevel {
        ...
        set {
            ...
            while (sources.MoveNext()) {
                ...
                SetGroupVolume(group.GameObjectName, group.groupMasterVolume); // set to same volume, but it recalcs based on master volume level.
            }
        }
    }
    ...
    public static SystemLanguage DynamicLanguage {
        get {
            ...
            return (SystemLanguage)Enum.Parse(typeof(SystemLanguage), PlayerPrefs.GetString(StoredLanguageNameKey));
        }
        set {
    ...
    public static bool UseDbScaleForVolume {
        ...
        set {
            PlayerPrefs.SetInt(UseDbKey, value ? 1 : 0);
        }
    }
    ...
    public static bool UseCentsForPitch {
        ...
        set {
            PlayerPrefs.SetInt(UseCentsPitchKey, value ? 1 : 0);
        }
    }
...
#if UNITY_EDITOR
        var pc = Resources.LoadAssetAtPath(MasterAudioFolderPath + "/Prefabs/SoundGroupOrganizer.prefab", typeof(GameObject));
#else
        var pc = Resources.Load(MasterAudioFolderPath + "/Prefabs/SoundGroupOrganizer.prefab", typeof(GameObject));
#endif
        if (pc == null) {
            Debug.LogError("Could not find SoundGroupOrganizer prefab. Please drag it into the scene yourself. It is located under MasterAudio/Prefabs.");
            return null;
        }
        // ReSharper disable once ArrangeStaticMemberQualifier
        var go = GameObject.Instantiate(pc) as GameObject;
