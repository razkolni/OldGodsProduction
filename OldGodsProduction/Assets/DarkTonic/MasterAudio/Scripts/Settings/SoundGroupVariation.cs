...
public class SoundGroupVariation : MonoBehaviour {
    ...
    void Awake() {
        ...
        if (VarAudio.playOnAwake) {
            Debug.LogWarning("The 'Play on Awake' checkbox in the Audio Source for Sound Group '" + ParentGroup.name + "', Variation '" + name + "' is checked. This is not used in Master Audio and can lead to buggy behavior. Make sure to uncheck it before hitting Play next time. To play ambient sounds, use an EventSounds component and activate the Start event to play a Sound Group of your choice.");
        }
    ...
    private void FinishSetupToPlay() {
        ...
        VarAudio.loop = AudioLoops; // restore original loop setting in case it got lost by loop setting code below for a previous play.
        ...
        var useClipAgePriority = MasterAudio.Instance.prioritizeOnDistance && (MasterAudio.Instance.useClipAgePriority || ParentGroup.useClipAgePriority);
    ...
    public MasterAudioGroup ParentGroup {
        get {
            ...
            if (_parentGroupScript == null) {
                Debug.LogError("The Group that Sound Variation '" + name + "' is in does not have a MasterAudioGroup script in it!");
            }
    ...
    public float OriginalPitch {
        get {
            if (original_pitch == 0f) { // lazy lookup for race conditions.
                original_pitch = VarAudio.pitch;
            }
