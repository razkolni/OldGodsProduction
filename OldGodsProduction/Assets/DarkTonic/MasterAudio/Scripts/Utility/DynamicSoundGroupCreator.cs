...
public class DynamicSoundGroupCreator : MonoBehaviour {
    ...
    public void CreateItems() {
        if (_hasCreated) {
            Debug.LogWarning("DynamicSoundGroupCreator '" + transform.name + "' has already created its items. Cannot create again.");
            return;
        }
        ...
        for (var i = 0; i < groupBuses.Count; i++) {
            ...
            if (aBus.isExisting) {
                ...
                if (confirmBus == null) {
                    MasterAudio.LogWarning("Existing bus '" + aBus.busName + "' was not found, specified in prefab '" + name + "'.");
                }
        ...
        for (var i = 0; i < customEventsToCreate.Count; i++) {
            var anEvent = customEventsToCreate[i];
            MasterAudio.CreateCustomEvent(anEvent.EventName, anEvent.eventReceiveMode, anEvent.distanceThreshold, errorOnDuplicates);
        }
    ...
    public static int HardCodedBusOptions {
        get {
            return MasterAudio.HardCodedBusOptions + ExtraHardCodedBusOptions;
        }
    }
